...
/**
* Standard output user interface for the College class, 
* which is a collection of Students. 
...
*   <li> Adding the grade point value and credit units 
*        earned for a course taken by an existing student.</li>
...
{
   /**
   * Opens the input stream, processes command to process information
   * on the students in the college and prints results to standard output.
   ...
   * with y or Y, we say that it IS interactive, otherwise, it is not.
   ...
   */
   public static boolean isInteractiveModeFromUser(Scanner in)
   {
      System.out.print(systemPrompt() + 
         "Is the session interactive? type \"yes\": ");
   ...
   /**
   * Prints to standard output the string str to be  used if the session is 
   * not interactive to show what the input was.
   * @param str string to print to standard output
   */
   ...
   */
   ...
   /**
   * Prints out a prompt to standard output and reads the next line. 
   * It continues reading as long as the line read is empty.
   ...
   */
   ...
   /**
   * Processes the commands that it gets using in and it asks the college
   * to carry out various functions.
   * <p> Precondition: the college is not null. </p>
   * @param college the College that will have data manipulated
   * @param in an open input stream
   * @param notInteractive flag which when true, does not echo the input
   */
   public static void processCommands(College college, 
   ...
   /**
   * Creates a student and adds it to the college. It asks the user from
   * stnadard input for a first and last name in a single line and 
   * subsequently it asks for an address (that is free formatted and may 
   * have blanks).
   * <p> Precondition: the college is not null. </p>
   * @param college the college of Students
   * @param in an open input stream
   ...
   */
   public static void add(College college,Scanner in, boolean notInteractive)
   {
      ...
      else
      {
         ...
         if (notInteractive) echo(address);
   ...
   /**
   * Finds and prints to standard output the student given an student number
   * which is obtained from standard input. 
   * If the student number given by the user is erroneous or if the student 
   * is not in the college, an error message is given onto standard output.
   * <p> Precondition: the college is not null. </p>
   * @param college the college of Students
   * @param in an open input stream
   * @param notInteractive declares that the session is not 
   ...
   */
   ...
   {
      System.out.print(systemPrompt() + "Enter the student number you would like to search(INTEGER PLEASE) ");
      String number = in.nextLine();
      if (notInteractive) echo( number );
      if (!isNumeric( number ))//number entered is not integer and this will print error on the console
      {
         String errorMsg = "\"" + number + "\" should be a student number ";
         printError(errorMsg);
      }
      else//it will find for the student in the college list 
      {
         Student stu = college.find( Integer.parseInt(number) );
         printStudent("", stu);
      }
   ...
   /**
   * Obtains and prints to standard output the login id of a student whose
   * student number is obtained from standard input. If the student number 
   * given by the user is erroneous or if the student is not in the college,
   * an error message is printed onto standard output.
   * <p> Precondition: the college is not null. </p>
   * @param college the college of Students
   * @param in an open input stream
   * @param notInteractive declares that the session is not 
   ...
   */
   ...
   /**
   * Deletes the student whose student number is obtained from standard input.
   * The removed student is printed to standard output or an error message
   ...
   */
   ...
   {
      System.out.print(systemPrompt() + "Enter the student number you would like to delete(INTEGER PLEASE) ");
      String number = in.nextLine();
      if (notInteractive) echo( number );
      if (!isNumeric( number ))//number entered is not integer and this will print error on the console
      {
      ...
      {
         Student stu = college.remove( Integer.parseInt(number) );
         printStudent("", stu);
      }
   } // remove

   /**
   * Asks the user using standard input for the student number and if the 
   * student number is valid, it asks the user for the credits of a course
   * (which should be numeric and positive) and if there are no errors,
   * it asks for the points for the course (which should be numeric, 
   * positive and an integer).
   * The credits and points are used to calculate the student's GPA. 
   * The errors are reported to standard output.
   * <p> Precondition: the college is not null. </p>
   * @param college the college of Students
   * @param in an open input stream
   * @param notInteractive declares that the session is not 
   ...
   */
   public static void addCourse(College college, Scanner in, boolean notInteractive) 
   {
      ...
      String inputStr = in.nextLine();
      if (notInteractive) echo(inputStr);
      ...
      {
         System.out.print(systemPrompt() + 
            "Enter the credit units for the course: ");
   ...
   /**
   * Prints one of the students with the college's highest GPA
   * given that there might be more than one student with the highest GPA:
   * only one is printed. If the college does not have any students
   ...
   */
   ...
   {
      if ( college.getTotalNumberOfStudents() == 0 ){
         printError(" No student in the college yet ");
         }
      printStudent( "with Highest GPA is ", college.getHighestGPAStudent() );
   } // findHighestGPA
   ...
   */
   public static void giveTotal(College college)
   {
    ...
    System.out.println( "Total number of students in the college is " + college.getTotalNumberOfStudents() );
   } // giveTotal
   ...
   * <p> Precondition: the college is not null and <code>toString</code> has
   * been defined both for <code>Student</code> and for <code>College</code>. 
   * </p>
   * @param college the college of Students
   */
   public static void printOutCollege(College college)
   {
      System.out.println( college.to_String() );
   } // printOutCollege
   ...
   * @param what gives what was done to the student, e.g. "added", "removed"
   * @param stud whose information will be printed.
   */
   ...
   /**
   * Obtains from standard input the student number and if it is valid,
   * it returns the student from the college with that student number.
   * <p> Precondition: the college is not null. </p>
   * @param college the college of Students
   * @param in an open input stream
   * @param notInteractive declares that the session is not 
   * with the console but likely from redirected input from a file
   * @return the student with the student number read from the scanner in
   * or null if the student number is erroneous or no student with that
   * student number is found in the college.
   */
   public static Student getStudent(College college, Scanner in,
   ...
   /**
   * Asks the user using standard input to enter a student number. 
   * If the student number is not numeric, report the error.
   * @param in an open input stream
   * @param notInteractive declares that the session is not 
   * with the console but likely from redirected input from a file
   * @return the student number if it is numeric and
   ...
   */
   ...
   /**
   * Asks the user using standard input for the points which should be numeric 
   * and positive: report errors to standard output
   * @param in an open input stream
   * @return the points obtained from the user 
   ...
   * @param in an open input stream
   * @return the credits obtained from the user 
   * or -1 if an error was encountered
   */
   ...
   * <p> Precondition: str is not null. </p>
   * @param str string to be examined for a blank inbetween
   * @return true if str has exactly two words with at least one blank 
   * between the words, false otherwise
   */
   ...
   * <p> Precondition: str is not null. </p>
   * @return true if str consists of characters that are digits, 
   * false otherwise
   * @param str has characters that should be numeric but might not be
   * @return true if every character of str is a digit or str is the empty
   * string, false otherwise 
   * (no concession is made for '+' or '-'
   */
   public static boolean isNumeric(String str)
   {
      ...
      for ( int i = 0; i< size; i++ ){
         x = str.charAt( i );
         if ( x < '0' || x > '9' ){// char is anything other than the numbers 0-9
            return false;
         }
      }
      return true;
